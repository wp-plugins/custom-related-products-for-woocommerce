...
function crp_select_related_products() {
	...
	<div class="options_group">
		...
		<select id="related_ids" name="related_ids[]" class="ajax_chosen_select_products" multiple="multiple" data-placeholder="<?php _e( 'Search for a product&hellip;', 'woocommerce' ); ?>">
			<?php
				$related_ids = get_post_meta( $post->ID, '_related_ids', true );
				$product_ids = ! empty( $related_ids ) ? array_map( 'absint',  $related_ids ) : null;
				if ( $product_ids ) {
			...
			?>
		</select> <img class="help_tip" data-tip='<?php _e( 'Related products are displayed on the product detail page.', 'woocommerce' ) ?>' src="<?php echo WC()->plugin_url(); ?>/assets/images/help.png" height="16" width="16" /></p>
	</div>
	<?php
}
